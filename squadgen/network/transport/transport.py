...
class Transport:
    ...
    def training_losses(
        ...
        model_output_dict = model(xt, t, **model_kwargs)
        model_output = model_output_dict['x']
        B, *_, C = xt.shape
        ...
        terms['score'] = self.get_score_from_model_output(xt, t, model_output)
        return terms
    ...
    def get_drift(
        ...
        def score_ode(x, t, model, **model_kwargs):
            ...
            model_output_dict = model(x, t, **model_kwargs)
            model_output = model_output_dict['x']
    ...
    def get_score_from_model_output(
        self,
        x,
        t,
        model_output
        ...
        """member function for obtaining score of 
            x_t = alpha_t * x + sigma_t * eps"""
        if self.model_type == ModelType.NOISE:
            score = model_output / -self.path_sampler.compute_sigma_t(path.expand_t_like_x(t, x))[0]
        elif self.model_type == ModelType.SCORE:
            score = model_output
        elif self.model_type == ModelType.VELOCITY:
